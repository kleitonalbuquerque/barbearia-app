...
export default function ProfessionalsPage() {
	...
	useEffect(() => {
		if (!authLoading && isAuthenticated) {
			setLoading(true);
			fetch(`/api/professionals?q=${encodeURIComponent(search)}&orderBy=${orderBy}&orderDir=${orderDir}&page=${page}&pageSize=${pageSize}`)
				.then((res) => res.json())
				.then((data) => {
		...
		<div className="w-full px-2 sm:px-4 md:px-8 lg:px-16 xl:px-32 py-4 mx-auto">
			<div className="flex items-center justify-between mb-6">
				...
				<button
					...
					onClick={() => router.push("/professionals/novo")}
				>
					+ Novo Profissional
				</button>
			</div>
