...
export default function ProfessionalsPage() {
	...
	// Extrai o tenant da URL: /[tenant]/professionals
	...
	useEffect(() => {
		if (!authLoading && isAuthenticated && tenant) {
			setLoading(true);
			fetch(`/api/professionals?q=${encodeURIComponent(search)}&orderBy=${orderBy}&orderDir=${orderDir}&page=${page}&pageSize=${pageSize}&tenantId=${tenant}`)
				.then((res) => res.json())
				.then((data) => {
	...
	return (
		<div className="w-full px-2 sm:px-4 md:px-8 lg:px-16 xl:px-32 py-4 mx-auto">
			<div className="flex items-center justify-between mb-6">
				<h1 className="text-2xl font-bold brand-title">Profissionais</h1>
				<button
					className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded shadow transition"
					onClick={() => router.push(`/${tenant}/professionals/novo`)}
				>
					+ Novo Profissional
				</button>
			</div>
